...
LLM output to code execution: LLM-generated content passed to `eval()`, `exec()`, `Function()`, `child_process.exec()`, or shell execution without validation — distinct from output sanitization (HTML rendering). Severity: Critical (RCE). Detection: grep for `eval.*completion|exec.*response|Function.*llm|spawn.*ai|child_process.*chat`.
System prompt leakage: model responses returned to users without checking for system prompt fragments — attacker crafts input to make model echo system instructions. Severity: Major. Detection: grep for system prompt strings stored in variables/constants and check if response filtering exists before returning LLM output to client.
