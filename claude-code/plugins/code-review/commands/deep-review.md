...
Provide a deep 9-agent code review for specific files. If a file has uncommitted changes, review those changes. If a file has no uncommitted changes, review the entire file.

**Use this when**: You need the most comprehensive review of specific files.

**Required argument**: One or more file paths to review (space-separated)

Example usage:
- `/deep-review src/utils.ts`
- `/deep-review src/api/handler.ts src/models/user.ts`
- `/deep-review --output-file review.md src/feature.ts`
- `/deep-review --language nodejs src/feature.ts` (force Node.js/TypeScript checks)
- `/deep-review --language dotnet src/Service.cs` (force .NET/C# checks)
